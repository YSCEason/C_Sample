...
/*======================================================================
 ...
 (C) Copyright 1997-2004;     ARC International;  Santa Cruz, CA 95060
...
enum {ARCINT_BASE_VERSION = 1,
      ...
      // as in an ICE trace buffer, which SeeCode's simulator maintains
      // anyway.
...
#define ARC_FEATURE_new_watchpoint_interface 64
#define ARC_FEATURE_handle_command 128
#define ARC_FEATURE_unused 256
#define ARC_FEATURE_additional_regs 512

// For new watchpoint interface:
#define ARC_WATCHPOINT_read 1	// WP on read
#define ARC_WATCHPOINT_write 2	// WP on write
...
struct ARC_reg_in_bank {
    const char *name;
    unsigned number;
    // Interface version 2:  
    // bit 0 == 1 => don't show if changes on single-step.
    // bit 4 == 1 => register is volatile and must be re-read each time
    //               it is requested (i.e. can't be cached).
    //		     Note that the debugger may read it multiple times
    //		     for formatting reasons.
    unsigned flags;
    unsigned size;	// Non-zero size overrides bank size.
    };

struct ARC_bank {
    const char *name;
    unsigned regsize;
    // Null-terminated list of registers in this bank:
    struct ARC_reg_in_bank **regs;
    };

struct ARC_additional_regs {     
    unsigned version;	// version of this interface.  Latest version: 2.
    // Null-terminated list of banks:
    struct ARC_bank **banklist;
    };
...
#define ARC_functions(func,fbdy,OBJ0,OBJ) \
    ...
    fbdy(void, receive_callback, (OBJ struct ARC_callback*), return;)	\
    ...
    fbdy(int, set_reg_watchpoint2, (OBJ ARC_REG_TYPE r, int length, unsigned options, void **cookie), return 0; )    \
    fbdy(int, remove_watchpoint, (OBJ void *cookie), return 0; ) \
    fbdy(int, set_mem_watchpoint2, (OBJ unsigned long addr, int length, unsigned options, void **cookie), return 0; ) \
    fbdy(int, stopped_at_watchpoint2, (OBJ void **cookie), return 0; )\
    fbdy(int, handle_command, (OBJ const char *cmd, const char *args), \
    	return 0; )					\
    fbdy(struct ARC_additional_regs*, additional_registers, (OBJ0), return 0;)\
...
enum {
    reg_LP_COUNT = 60,
    reg_ISS_ICNT = 61,	// ICNT in MetaWare ISS.
    reg_CYCLES = 62,	// cycles in MetaWare ISS or in some simulators.
    AUX_BASE = 64,
    ...
    reg_bank_aux  = 1,
    reg_bank_madi = 2,
    // For registers returned from ARC_reg_in_bank are all read using
    // bank 64.
    reg_bank_extra  = 64,
    };
...
#define ARC_callback_functions(func,OBJ0,OBJ) \
    ...
    /* Version 3 interface ends here. */		\
    /* get_debugger_access isn't valid until a process has been created. \
       We have added get debugger_installation_directory to alleviate this \
       problem for DLLs that need to know where the debugger was installed \
       early. */					\
    func(struct Debugger_access *, get_debugger_access, (OBJ0))\
    /* get_system is private; for debugger use only: */		\
    func(void *, get_system, (OBJ0))			\
    /* The target's state has been invalidated.  Restart is necessary */ \
    /* to proceed.  Can happen, e.g., when memory is resized in */ 	\
    /* a simulator. The debugger kills the current process. */		\
    /* If options & 1, the debugger will also perform process restart.*/\
    /* automatically, rather than requiring the user to do it. */  	\
    func(void, target_state_invalidated, (OBJ unsigned options))	\
    func(const char*, debugger_installation_directory, (OBJ0))\
    /* Version 4 interface ends here. */		\
    /* Target state may have changed more than expected, such as        \
       a register or memory change the debugger did not request.        \
       The debugger will attempt to update displays accordingly.        \
       The options parameter is presently not used.			\
     */									\
    func(void, target_state_changed, (OBJ unsigned options))		\
    /* Version 5 interface ends here. */		\
    func(int, eprintf, (OBJ const char *format, ...)) 	\
    func(int, evprintf, (OBJ const char *format, va_list ap)) 	\
    /* Version 6 interface ends here. */		\
...
struct sim_disarc {
    virtual char *disarc(
    	// target addr (unxlated) vs maddr (xlated: real memory).
    	unsigned taddr, unsigned maddr, unsigned long *memory) = 0;
    };
struct ARC_simext;
struct Addr_counter;
struct Trace_history;
struct Text_display_created;
struct Additional_metaware_simulator_communication {
    ...
    virtual unsigned get_memalloc_limit() = 0;
    virtual Text_display_created *cache_info(int icache) = 0;
    virtual void prefetch_memory(unsigned adr, void *, unsigned amount) = 0;
    // Index by 0..31: if deleted[i] == 1, it's gone.
    virtual void specify_deleted_registers(unsigned *deleted) = 0;
    };
...
version of the interface can be generated.  SeeCode uses the C++
...
in C++ for SeeCode.  We have not see any GNU C++ that can, as GNU C++ typically
uses a virtual function table format that cannot be made compatible.)
...
is_simulator	-- tell if you are a simulator, and your characteristics
    ...
    This function tells the debugger whether the processor is
...
run	-- initiate execution
    ...
    Starts running the CPU.  Returns 1 if succeeded, 0 if failed. Hardware
    should be started running and the function must immediately
...
read_memory -- Read processor memory
    ...
    invalidate builds of old C source code that used the prior interface;
    however, prior implementations are not invalidated, due to the C
    calling convention, which has the caller popping the arguments from
    ...
    from your copy of arcint.h.  Worse, for C++ code, the change of the 
    interface will cause the previously-overriding read_memory function
    to be ignored and no longer override the base function. 
    MetaWare High C/C++ has the "override" keyword to enforce an override.
    For C++, read else where in this document for the use of CHECK_OVERRIDES.
...
set_memory_size	-- Specify memory size
    ...
    Generally a size of S means memory 0..S-1 is valid.  However, for
    the SeeCode ARC simulator, S means the amount of memory the simulator
    will (dynamically) allocate to accommodate a program.  The dynamically
    allocated memory can be scattered throughout the address space.
    This makes it easy, for example, to link a program at 0x8000_0000
    as well as define interrupt vectors at 0.  For the SeeCode simulator,
    the default value memory size is large -- 32MB -- and is allocated
    in 4K pages.
...
prepare_for_new_program	-- Load and restart notification
    ...
    a previous run.  But the function need not flush the pipe or
...
process_property -- Receive property settings
    ...
    debugger hasn't otherwise recognized. The property is named by "key"
    and contains the value given by the "value" input.  If your interface
    ...
    interface.  The debugger uses this to set timeout and port values to
...
additional_possibilities	-- Identify additional data about the interface
    ...
    Currently Seecode uses this function to detect that the debugger is
    talking to SeeCode's simulator or ARC hardware interface, and does
...
at_breakpoint	-- Tell whether at breakpoint
    ...
    If we are not at a breakpoint, return 0.
    If we are at a breakpoint: 
	In the old method, return 1.
	    The PC is assumed to point to the location of the broken
	    instruction.  
	In the new method, see below for what to return.
...
The following functions are functional replacements and improvements
for the watchpoint mechanism.  They allow read, write, and read/write
watchpoints.  The watchpoint identification is also done using a cookie
provided by the interface.  The debugger calls these functions only
if your version is >= ARCINT_TRACE_VERSION and you return
the ARC_FEATURE_new_watchpoint_interface bit from supports_feature.

int set_reg_watchpoint2
int remove_watchpoint
int set_mem_watchpoint2
int stopped_at_watchpoint2

    int set_reg_watchpoint2(
    	ARC_REG_TYPE r, int length, unsigned options, void **cookie)
    int remove_watchpoint(void *cookie);
    int set_mem_watchpoint2(
    	unsigned long addr, int length, unsigned options, void **cookie);
    int stopped_at_watchpoint2(void **cookie);

    The options parameter has a bitwise or of the values
	ARC_WATCHPOINT_read
	ARC_WATCHPOINT_write
    specifying which kind of watchpoint is desired.

    The set functions return a value in *cookie that the debugger uses
    to remove the watchpoint later.

    The stopped_at_watchpoint2 function provides the cookie if it returns 1.
...
handle_command		-- process command
    ...
    This is used for handling an arbitrary command that the command
    processor does not recognize.  It is called only if you include
    ARC_FEATURE_handle_command in the return value of supports_feature().

    Return 1 if you recognize the command, 0 otherwise.

additional_registers	-- Specify additional registers to SeeCode

    ARC_additional_regs* additional_registers

    Use this function to return a list of additional registers you would
    like SeeCode to display.  For example, a simulator could define
    a series of internal statistics registers such as cache hit and miss
    counters.
    
    This function is called only if you include ARC_FEATURE_additional_regs 
    in the return value of supports_feature().

    ARC_additional_regs specifies the version of this interface (currently
    at 2) and a null-terminated list of register banks (ARC_bank).  
    Each ARC_bank can be separately displayed by the debugger, 
    so you can group your added registers into separate banks for 
    convenience of display.

    ARC_bank specifies a null-terminated list of registers in the
    bank, and the asize of each register in the bank, although
    an individual register can have a size different from the rest
    in the bank, by specifying a non-zero size in ARC_reg_in_bank.

    ARC_reg_in_bank gives the name of each register and the number
    the debugger returns to you to access the register.

    Access to registers specified in this way is via the functions
    read_banked_reg and write_banked_reg with the value reg_bank_extra
    passed as the bank parameter to those functions.

    Apology: the notion of a logical bank (ARC_bank) is not the same
    as the "int bank" parameter in read/write_banked_reg.  Thus the
    value bank = reg_bank_extra no matter how many ARC_banks you specify.

    For example, if in an ARC_reg_in_bank you specify the number 34
    for your register, SeeCode calls read_banked_reg(reg_bank_extra, 34, ...)
    to obtain the value of the register.

    This mechanism is used by the ARC ISS to introduce simulator counter
    registers.
...
struct My_ARC: ARC {
    ...
    // Here's an example of supplying an address counter.
    struct Test_count : Addr_count {
	override int version() { return ADDR_COUNTINT_CLEAR_VERSION; }
	override const char *id() { return "Test counter"; }
	override void destroy() { delete this; }
	~Test_count() { }
	override int enabled() { return ADDRCOUNT_ENABLED_CODE | ADDRCOUNT_ONLY_CODE; }
	override const char *name() { return "Test"; }
	override const char *short_name() { return name(); }
	override int get_count(unsigned adr, unsigned *same_as, unsigned *next)
	    {
	    *same_as = adr & ~1;	// Even addresses only.
	    *next = adr+2;
	    return OK_addr(adr) ? adr & 0xff : 0;// Count = addr mod 256.  Kinda silly.
	    }
	int OK_addr(unsigned addr) {
	    return addr >= 0x10000 && addr <= 0x10100;
	    }
	// The count is for addresses 0x10000 and 0x10004.
	int last_addr;
	override int next_address(int init, unsigned *addr) {
	    unsigned ix;
	    if (init & 1) {
		last_addr = *addr = 0x10000;
		return 1;
		}
	    else {
		*addr = last_addr+=2;
		return OK_addr(last_addr);
		}
	    }
	override int clear() {
	    return 1;
	...
	if (counter == 0) {
	    counter = new Test_count();
	    callback->get_debugger_access()->add_addr_count(counter);
	    }
...
#endif
/*
// This can be built with MetaWare High C and the command:
// 	hc arcint.c -mslink -Hdll
